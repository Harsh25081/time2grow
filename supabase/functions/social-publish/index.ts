...
Deno.serve(async (req) => {
  ...
  try {
    ...
    for (const target of targets) {
      ...
      try {
        if (!handle) {
          throw new HttpError(400, 'This is a demo target. Add and select a saved real handle before publishing.');
        }
        ...
        results.push({ targetId: target.id, provider, label: target.target_label, status: 'failed', error: message });
      }
    }
...
async function publishProvider({
  ...
  switch (provider) {
    ...
    case 'google_ads':
      throw new HttpError(400, 'Google Ads is not a social post target. Build this from the Ads campaign flow with budget, objective, assets, and policy review.');
    default:
...
async function publishFacebook(post: Record<string, unknown>, handle: Record<string, unknown>, asset: Record<string, unknown> | null, supabase: ReturnType<typeof serviceClient>) {
  ...
  return { externalId: stringValue(response.id) ?? stringValue(response.post_id), response };
}

async function publishInstagram(post: Record<string, unknown>, handle: Record<string, unknown>, asset: Record<string, unknown> | null, supabase: ReturnType<typeof serviceClient>) {
  if (!asset) throw new HttpError(400, 'Instagram publishing requires an image or video file.');

  const igUserId = externalId(handle, 'Instagram Business Account ID');
  const token = await providerAccessToken(supabase, 'instagram', stringValue(post.org_id) ?? '', handle);
  const graph = graphBase();
  ...
  const published = await formPost(`${graph}/${igUserId}/media_publish`, { access_token: token, creation_id: creationId });

  return { externalId: stringValue(published.id), response: published };
}

async function publishLinkedIn(post: Record<string, unknown>, handle: Record<string, unknown>, asset: Record<string, unknown> | null, supabase: ReturnType<typeof serviceClient>) {
  const token = await providerAccessToken(supabase, 'linkedin', stringValue(post.org_id) ?? '', handle);
  ...
  if (asset) {
    ...
    if (media.mimeType.startsWith('video/')) {
      throw new HttpError(400, 'LinkedIn video upload needs the Videos API finalize flow. Use poster/text now or add the video flow next.');
    }
  ...
  if (!response.ok) throw new HttpError(response.status, await responseText(response, 'LinkedIn publish failed.'));

  return {
    externalId: response.headers.get('x-restli-id'),
    response: { id: response.headers.get('x-restli-id'), status: response.status },
  };
}

async function publishYouTube(post: Record<string, unknown>, asset: Record<string, unknown> | null, supabase: ReturnType<typeof serviceClient>) {
  if (!asset || !mimeType(asset).startsWith('video/')) {
    throw new HttpError(400, 'YouTube publishing requires a video file.');
  }

  const token = await oauthAccessToken(supabase, 'youtube', stringValue(post.org_id) ?? '');
  const media = await downloadMedia(supabase, asset);
...
async function publishWhatsApp(post: Record<string, unknown>, handle: Record<string, unknown>, asset: Record<string, unknown> | null, supabase: ReturnType<typeof serviceClient>) {
  const token = await providerAccessToken(supabase, 'whatsapp', stringValue(post.org_id) ?? '', handle);
...
async function publishSlack(post: Record<string, unknown>, target: Record<string, unknown>, handle: Record<string, unknown>, asset: Record<string, unknown> | null, supabase: ReturnType<typeof serviceClient>) {
  const token = await providerAccessToken(supabase, 'slack', stringValue(post.org_id) ?? '', handle);
  const channel = stringValue(handle.external_handle_id) || stringValue(target.target_label);
  if (!channel) throw new HttpError(400, 'Slack channel ID is missing.');

  const text = asset ? `${postMessage(post)}\n${await signedMediaUrl(supabase, asset)}` : postMessage(post);
  const response = await fetchJson('https://slack.com/api/chat.postMessage', {
    method: 'POST',
    headers: bearerJsonHeaders(token),
    body: JSON.stringify({ channel, text, unfurl_links: true, unfurl_media: true }),
  });

  if (response.ok !== true) throw new HttpError(400, `Slack rejected the message: ${stringValue(response.error) ?? 'unknown_error'}`);
  return { externalId: stringValue(response.ts), response };
}

async function publishTelegram(post: Record<string, unknown>, handle: Record<string, unknown>, asset: Record<string, unknown> | null, supabase: ReturnType<typeof serviceClient>) {
  const token = await providerAccessToken(supabase, 'telegram', stringValue(post.org_id) ?? '', handle);
  ...
  const response = await fetchJson(`${baseUrl}/${isVideo ? 'sendVideo' : 'sendPhoto'}`, {
    method: 'POST',
    body: form,
  });
...
async function providerAccessToken(supabase: ReturnType<typeof serviceClient>, provider: Provider, orgId: string, handle?: Record<string, unknown>) {
...
async function oauthAccessToken(supabase: ReturnType<typeof serviceClient>, provider: Provider, orgId: string) {
  const { data, error } = await supabase
  ...
  if (error) throw error;
  if (!data) throw new HttpError(400, `Connect ${provider} before publishing.`);
...
function tokenStillUsable(expiresAt: unknown) {
  const value = stringValue(expiresAt);
  return !value || Date.parse(value) > Date.now() + 60_000;
}

async function downloadMedia(supabase: ReturnType<typeof serviceClient>, asset: Record<string, unknown>) {
  ...
  if (error || !data) throw new HttpError(400, error?.message ?? 'Could not download media.');
...
async function signedMediaUrl(supabase: ReturnType<typeof serviceClient>, asset: Record<string, unknown>) {
  const bucket = stringValue(asset.storage_bucket) ?? 'post-media';
  const path = stringValue(asset.storage_path);
  if (!path) throw new HttpError(400, 'Media storage path is missing.');

  const { data, error } = await supabase.storage.from(bucket).createSignedUrl(path, 60 * 60);
  if (error || !data?.signedUrl) throw new HttpError(400, error?.message ?? 'Could not create media URL.');
  return data.signedUrl;
}

async function waitForInstagramContainer(graph: string, containerId: string, token: string) {
  for (let attempt = 0; attempt < 6; attempt += 1) {
    const status = await fetchJson(`${graph}/${containerId}?fields=status_code&access_token=${encodeURIComponent(token)}`);
    if (status.status_code === 'FINISHED') return;
    if (status.status_code === 'ERROR') throw new HttpError(400, 'Instagram could not process this video.');
    await new Promise((resolve) => setTimeout(resolve, 5000));
  }

  throw new HttpError(408, 'Instagram video is still processing. Try publishing again in a minute.');
}
...
async function uploadLinkedInImage(token: string, owner: string, blob: Blob) {
  ...
  if (!uploadUrl || !image) throw new HttpError(502, 'LinkedIn did not return an image upload URL.');
  ...
  if (!upload.ok) throw new HttpError(upload.status, await responseText(upload, 'LinkedIn image upload failed.'));
  return image;
}
...
function providerToken(provider: Provider) {
...
function envValue(name: string) {
  ...
  if (!value) throw new HttpError(400, `${name} is not configured in Supabase secrets.`);
  return value;
}
...
function telegramMessageId(response: Record<string, unknown>) {
  const id = response.result && typeof response.result === 'object' ? (response.result as Record<string, unknown>).message_id : null;
